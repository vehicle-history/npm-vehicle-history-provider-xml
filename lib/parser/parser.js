...
var getResultMap = function (result, mapperDefaults, mappers, callback) {
  ...
  for (var j in mappers) {
    if (mappers.hasOwnProperty(j)) {
      var mapper = mappers[j];
...
exports.generateReport = function (body, searchCarRequest, options, callback) {
  ...
  return getResultMap(data.result, mapperDefaults, mappers, function (err, map) {
    if (err) {
      logger.error('unable to parse car data:', err);
      return callback(err);
    }

    return resolver.resolver(map, searchCarRequest, options, function (errResolver, updatedMap) {
      if (errResolver) {
        logger.error('unable to resolve data: %s', errResolver);
        return callback(errResolver);
      }

      return responseBuilder.build(updatedMap, options, function (errResponseBuilder, report) {
